export default {
  ...
  methods: {
    ...
    getMiniPageList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/platform/shopTemplate/pageMove'),
        methods: 'get',
    ...
    copyTemplatePage (templateId) {
      this.$http({
        url: this.$http.adornUrl(`/platform/shopTemplate/copyMove/${templateId}`),
        method: 'POST'
      }).then(({ data }) => {
    ...
    handleDelete (renovationId) {
      this.$confirm(this.$i18n.t('shopFeature.list.deleteTips'), this.$i18n.t('shopFeature.list.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/platform/shopTemplate/deleteMove/' + renovationId),
          method: 'delete'
        }).then(({ data }) => {
