...
export default {
  ...
  methods: {
    ...
    getDetail (id) {
      let that = this
      this.$http({
        url: this.$http.adornUrl(`/platform/shopRenovation/info/${id}`),
        method: 'get'
      }).then(async ({data}) => {
        document.title = data.name
        let useArr = []
...
.select-decorate-page {
    ...
    overflow-y: auto;
    // 头部样式
    .store-signate-page-container .store-singate-content {
        width: 1200px!important;
    }
    .store-top .top-search .search-input {
        width: 600px!important;
    }
    .store-top .top-search .search-input .search-btns {
        width: 109px!important;
    }
    .store-top .top-search .search-input .search-types {
        right: 124px!important;
    }
    .store-top .top-search .top-cart {
