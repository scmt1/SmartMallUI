...
export default {
  ...
  methods: {
    ...
    getPlatformCategory () {
      this.loading = true
      const url = this.applyForSign ? `/platform/signingAuditing/listApplySigningCategory` : `/prod/category/platformCategory`
      this.$http({
        ...
        if (data && data.length) {
          this.visible = true
          this.$nextTick(() => {
            this.$refs['tableBox'].addEventListener('scroll', this.handleScroll, true)
          })
          ...
          this.$confirm('当前店铺已签约平台全部类目，暂无可新增签约类目', '提示', {
    ...
    handleCheckChange (nodeObj, nodeStatus) {
      ...
      if (nodeStatus) {
        if (this.signCategories.length + this.signingCount === 200) {
          if (!this.message) {
            this.message = true
            this.$message({
     ...
     */
    comfirmSubmit () {
      if (this.signCategories.length > 0) {
    ...
    signCategory () {
      ...
      const url = this.applyForSign ? '/platform/signingAuditing/addSigningCategory' : '/prod/category/signing'
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: params,
    ...
    handleNodeClick (nodeObj, nodeStatus) {
      ...
      let keys = this.signCategories.map(item => item.categoryId)
  ...
  .signing-categiries-content {
    ...
    .content {
      ...
      .right-box {
        ...
        // 上传图片样式
        .business-qual {
          & ::v-deep .mul-pic-upload .el-upload-list--picture-card .el-upload-list__item {
            width: 80px;
            height: 80px;
            line-height: 80px;
            margin-bottom: 0;
          ...
          & ::v-deep .mul-pic-upload .el-upload--picture-card {
            width: 80px;
            height: 80px;
            line-height: 80px;
            background: #fff;
