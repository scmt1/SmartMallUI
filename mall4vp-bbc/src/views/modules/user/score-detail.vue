    ...
    <el-table :data="dataList"
      ...
      <el-table-column prop="source" :label="$t('user.changeReason')">
        <template slot-scope="scope">
          ...
          <span v-else>{{[$t('user.registerScore'),$t('user.shopping'),$t('user.levelUp'),$t('user.signIn'),$t('user.shoppingDeducteScore'),$t('user.scoreExpired'),$t('user.rechargeBalance'),$t('user.sysChangeScore'),"抽奖","小票积分","小票冲红"][scope.row.source]}}</span>
...
export default {
  ...
  methods: {
    ...
    getData (page) {
      ...
      this.$http({
        url: this.$http.adornUrl('/user/userScoreLog/pageByUserId'),
        method: 'get',
