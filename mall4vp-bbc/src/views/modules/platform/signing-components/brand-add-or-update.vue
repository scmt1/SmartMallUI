<template>
  ...
  >
    <div class="popup">
      <div class="title">
        <div class="text">{{ applyForSign ? $t('shopProcess.addSigningBrand') : $t('shopProcess.editSigningBrand') }}</div>
        <div class="tips">
          {{ $t('shopProcess.preSigned') }}
          <span class="bold">{{ signingCount }}</span>
          {{ $t('shopProcess.piece') }}{{ $t('shopProcess.brand') }}，
          {{ $t('shopProcess.application') }}
          <span class="bold">{{ applyingBrandNum }}</span>
          {{ $t('shopProcess.piece') }}{{ $t('shopProcess.brand') }}，
        ...
        <div class="left-box">
          ...
          <div class="brand-box">
            <div class="brand-list">
              ...
              <div v-if="!allBrandList || !allBrandList.length"  class="brands-empty">{{ $t('order.noData') }}</div> 
            </div>
          </div>
        </div>
        <div class="right-box">
...
export default {
  ...
  methods: {
    ...
    getPlatformBrand () {
      this.$http({
        url: this.applyForSign ? this.$http.adornUrl(`/platform/signingAuditing/listApplySigningBrand`) : this.$http.adornUrl(`/admin/brand/list`),
        method: 'get',
        params: this.$http.adornParams({ name: this.keyword, status: 1, shopId: this.shopId })
      }).then(({ data }) => {
        if (data && data.length) {
          this.visible = true
          ...
          this.$confirm('当前店铺已签约平台全部品牌，暂无可新增签约品牌', '提示', {
    ...
    signBrand () {
      ...
      this.$http({
        url: this.applyForSign ? this.$http.adornUrl('/platform/signingAuditing/addSigningBrand') : this.$http.adornUrl('/admin/brand/signing'),
        method: 'post',
        data: params.platformBrandList,
        params: this.$http.adornParams({
          shopId: this.shopId
        })
      }).then(({ data }) => {
