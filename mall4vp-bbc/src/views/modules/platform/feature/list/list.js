...
export default {
  ...
  methods: {
    ...
    getMiniPagesList () {
      this.$http({
        url: this.$http.adornUrl('/platform/shopRenovation/pageMove'),
        method: 'get',
    ...
    handleDelete (item) {
      this.$confirm(this.$i18n.t('shopFeature.list.deleteTips'), this.$i18n.t('shopFeature.list.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/platform/shopRenovation/operate/deleteMove/' + item.renovationId),
          method: 'delete'
        }).then(({ data }) => {
    ...
    handleSetHomePage (renovationId) {
      this.$http({
        url: this.$http.adornUrl('/platform/shopRenovation/operate/updateHomePageMove/' + renovationId),
        method: 'put'
      }).then(({ data }) => {
