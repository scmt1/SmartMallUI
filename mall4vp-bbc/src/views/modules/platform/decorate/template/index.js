export default {
  ...
  methods: {
    ...
    getMiniPageList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/platform/shopTemplate/pagePC'),
        methods: 'get',
    ...
    copyTemplatePage (templateId) {
      this.$http({
        url: this.$http.adornUrl(`/platform/shopTemplate/copyPC/${templateId}`),
        method: 'post'
      }).then(({ data }) => {
    ...
    handleDelete (renovationId) {
      this.$confirm(this.$i18n.t('shopFeature.list.deleteTips'), this.$i18n.t('shopFeature.list.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/platform/shopTemplate/deletePC/' + renovationId),
          method: 'delete'
        }).then(({ data }) => {
