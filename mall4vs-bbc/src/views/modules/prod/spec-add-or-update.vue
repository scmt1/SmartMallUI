<template>
  ...
  >
    ...
    <el-form-item :label="$t('product.chooseLanguage')">
    ...
    <el-form-item :label="$t('product.attributeName')">
        <el-table :data="dataList" border :style="{width: value1[0]=='En'||value1[1]=='En'?'450px':'225px'}" size="medium" :show-header='false'>
      <el-table-column
        prop="propName"
        header-align="center"
        align="center"
      ...
      >
        <template slot-scope="scope">
          <el-input :placeholder="$t('product.attributeName')" v-model="scope.row.propName" maxlength="10"
                    size="small"
                    show-word-limit
                    clearable
      ...
      <el-table-column v-if="value1[0]=='En'||value1[1]=='En'" prop="propNameEn"
                       header-align="center"
                       align="center"
  ...
  <el-form-item >
    <el-table :data="prodPropValuesData" max-height="225px" :style="{width: value1[0]=='En'||value1[1]=='En'?'650px':'390px'}"  :show-header='false' :border="false">
      <el-table-column
        prop="propValue"
...
export default {
  ...
  methods: {
    ...
    selectChange () {
      ...
      // 只有中文时，移除英文的内容
      if (this.value1.length <= 1) {
        this.dataList[0].propNameEn = ''
        this.prodPropValuesData.forEach(item => {
          item.propValueEn = ''
        })
      }
    },
    ...
    dataFormSubmit () {
      ...
      if (this.dataList[0].propName.indexOf(';') !== -1) {
        this.dataList[0].propName = ''
        this.$message.error(this.$i18n.t('product.doNotEnterSpecialCharacters'))
        return
      }
      if (this.dataList[0].propNameEn.indexOf(';') !== -1) {
        this.dataList[0].propNameEn = ''
        this.$message.error(this.$i18n.t('product.doNotEnterSpecialCharacters'))
        return
      }
      if (this.prodPropValuesData.some(item => (item.propValue && item.propValue.indexOf(';') !== -1) || (item.propValueEn && item.propValueEn.indexOf(';') !== -1))) {
        this.$message.error(this.$i18n.t('product.doNotEnterSpecialCharacters'))
        return
      }
      ...
      this.$http({
        ...
        data: this.$http.adornData({
          ...
          propNameEn: this.dataList[0].propNameEn || this.dataList[0].propName,
