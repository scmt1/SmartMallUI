<template>
  <div class="app-container-10 container-f0f2f5 app-main feature-list-container">
    <div class="feature-list-content container-fff app-container-15 app-ele-border-radius-0">
      <div class="default-btn primary-btn" v-if="isAuth('platform:shopRenovation:savePC')" @click="newPageSelect">{{$t('shopFeature.list.newMicroPage')}}</div>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="text-btn-con">
                ...
                <div class="default-btn text-btn" v-if="isAuth('platform:shopRenovation:updatePC')" @click="handleEdit(scope.row)">{{$t('shopFeature.list.edit')}}</div>
                <div class="default-btn text-btn" v-if="isAuth('platform:shopRenovation:deletePC')" @click="handleDelete(scope.row)">{{$t('shopFeature.list.delete')}}</div>
                <div class="default-btn text-btn update-home-page" v-if="isAuth('platform:shopRenovation:updateHomePagePC')" @click="handleSetHomePage(scope.row.renovationId)">{{ scope.row.homeStatus!=1 ? $t('shopFeature.list.setHomePage'): $t('shopFeature.list.cancelHomePage')}}</div>
              </div>
            </template>
          </el-table-column>
        </el-table>
