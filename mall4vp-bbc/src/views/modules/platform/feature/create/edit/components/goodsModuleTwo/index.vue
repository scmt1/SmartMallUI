  ...
  methods: {
    ...
    checkData () {
      ...
      if (isPass) {
        ...
        // 左标题禁止小数
    leftMainChange (val) {
      return this.formData.leftConfig.mainTitleSize = Math.floor(val)
    },
        // 左副标题禁止小数
    leftSubChange (val) {
      return this.formData.leftConfig.subTitleSize = Math.floor(val)
    },
        // 上标题
    topMainChange (val) {
      return this.formData.topConfig.mainTitleSize = Math.floor(val)
    },
        // 上副标题
    topSubChange (val) {
      return this.formData.topConfig.subTitleSize = Math.floor(val)
    },
        // 下标题禁止小数
    bottomMainChange (val) {
      return this.formData.bottomConfig.mainTitleSize = Math.floor(val)
    },
        // 下副标题禁止小数
    bottomSubChange (val) {
      return this.formData.bottomConfig.subTitleSize = Math.floor(val)
    }
  }
}
</script>
