                    ...
                    <el-form :inline="true" class="search-form" ref="searchForm" :model="searchForm" size="small">
                        <div class="input-row">
                            ...
                            <el-form-item>
                                ...
                                <div class="default-btn primary-btn" @click="settlement()">结算</div>
                            </el-form-item>
                        </div>
                    </el-form>
                </div>
...
<!--                        <el-table-column prop="shopAmount" label="实得金额（元）"/>-->
  ...
  export default {
    ...
    methods:{
      ...
      exportFrom(){
        ...
        this.searchForm.payType = 1
        this.$http({
          url: this.$http.adornUrl('/multishop/userBalanceUseLog/carRecordDownload'),
          method: 'GET',
          params: this.$http.adornParams(
            Object.assign({
              'startTime': this.dateRange === null ? null : this.dateRange[0], // 开始时间
      ...
      downloadCarRecord(){
        ...
        this.$http({
          url: this.$http.adornUrl('/multishop/carManage/downloadCarRecord'),
          method: 'GET',
          params: this.$http.adornParams({
            'extParam': this.carDataForm.carNo ? this.carDataForm.carNo : this.carDataForm.carType ? this.carDataForm.carType : '',
            'wayCode': this.carDataForm.wayCode,
