              ...
              <div class="elx-main elx-img-list">
                ...
                <div class="img-item-con">
                  <div v-for="(img, itemIndex) in imgRes.records" :key="itemIndex" class="img-item">
                      ...
                      <span class="selected-def" @click="onClickListImage(img)"></span>
                ...
                <div class="data-tips" v-show="imgRes.records!==undefined&&imgRes.records.length<1">{{$t("order.noData")}}</div>
                <el-pagination
  ...
  methods: {
    ...
    loadListImage (current, newData = false) {
      ...
      this.$http({
        ...
        this.imgRes = data
        let existCount = 0
        const imgResRec = this.imgRes.records || []
        imgResRec.forEach(img => {
          if (!this.isExist(img)) {
            img.selected = false
          } else {
            existCount++
        ...
        // 是否全选
        this.selectAll = imgResRec.length !== 0 && imgResRec.length === existCount
        if (existCount && imgResRec.length !== existCount) {
    ...
    // 检测是否已选中
    isExist (img) {
      return this.fileIds.includes(img.fileId)
    },
    /**
     * 图片已选则返回下标，未选则返回-1
     */
