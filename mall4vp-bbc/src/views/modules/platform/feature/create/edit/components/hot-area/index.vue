  ...
  <div class="micro-image-ad-box">
    ...
    <div class="design-preview-controller">
      <div v-if="formData.imageList.length"
        ...
        <div v-if="formData.indicator ===1 || formData.indicator ===5 ">
          ...
          <draggable :list="formData.imageList"
            ...
            <div v-for="(item,index) in formData.imageList"
              ...
              <div class="add-ad-image"
                    @click="showHotAreaPop(index)">{{$t('shopFeature.imageAd.addHotArea')}}</div>
              <div class="add-ad-image"
                    @click="changeImg(index)">{{$t('shopFeature.tabNav.changePic')}} </div>
              <div class="ad-image-hot-box">
                <div v-show="item.activeBoxs.length"
                  ...
                  <span v-for="(hotItem,hotIndex) in item.activeBoxs"
                    ...
                    <div class="redirect-title" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis">{{hotItem.title}}</div>
...
export default {
  ...
  data () {
    return {
      ...
      formData: {
        indicator: 6, // 选择模板: 1一行一个; 2轮播海报; 3横向滑动; 4一行4个; 5一行2个; 6热区
        imageList: [] // 图片列表
      },
  ...
  methods: {
    ...
    setLinkInfo (obj) {
      ...
      if (this.formData.indicator === 5) { // 热区非弹窗编辑模式
    ...
    handleDialogSubmit ({ type, value }) {
      if (type === '1') { // 当前选择的是商品
        ...
        this.cacheImageActiveBox[this.currentImgIndex].type = '3'
        this.cacheImageActiveBox[this.currentImgIndex].title = value.storeItem.shopName
        this.cacheImageActiveBox[this.currentImgIndex].link = value.storeItem.shopId
    ...
    checkData () {
      if (this.formData.imageList.length > 0) {
        ...
        // 弹窗提示错误消息
        this.showCheckForm()
        this.$newMessage.error(this.$i18n.t('shopFeature.imageAd.hotArea') + this.current + this.$i18n.t('shopFeature.tabNav.pleaseAddPic'))
      }
    }
  }
}
